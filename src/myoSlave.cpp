...
int32_t *MyoSlave::darkroom_base;
...
ros::Publisher MyoSlave::darkroom;
...
MyoSlave::MyoSlave(vector<int32_t*> &myobase, vector<int32_t*> &i2c_base, vector<int> &deviceIDs, int32_t* h2p_lw_darkroom_addr,
                   int argc, char* argv[]){
...
//
    ...
    darkroom = nh->advertise<roboy_communication_middleware::DarkRoom>("/roboy/middleware/DarkRoom/sensors", 1);
    ...
    darkroom_base = h2p_lw_darkroom_addr;
...
//
    ...
    for (uint motor = 0; motor < 14; motor++) {
        ...
        msg2.position.push_back(getPosition(motor));
        msg2.velocity.push_back(getVelocity(motor));
        msg2.displacement.push_back(getDisplacement(motor));
        msg2.current.push_back(getCurrent(motor));
    }
    motorStatus.publish(msg2);
    return ret;
}
